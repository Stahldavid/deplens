...
async function loadCore() {
  if (!corePromise) {
    corePromise = import('@deplens/core');
  }
  return corePromise;
}
...
async function loadDiff() {
  if (!diffPromise) {
    ...
    diffPromise = Promise.resolve(core.runDiff || core.default?.runDiff || null);
  }
  return diffPromise;
}
...
const inspectToolSchema = {
  ...
  properties: {
    ...
    analyzeSource: {
      type: 'boolean',
      description: 'Analyze source code (.ts/.js) for implementation details, complexity, patterns',
    },
    sourceMaxFiles: {
      type: 'number',
...
const diffToolSchema = {
  ...
  properties: {
    ...
    from: {
      type: 'string',
      description: "Source version (e.g. '3.22.0', 'installed'). Default: 'installed'",
    },
    to: {
      type: 'string',
      description: "Target version (e.g. '3.24.0', 'latest'). Default: 'latest'",
    },
    includeSource: {
      type: 'boolean',
...
const diffToolOutputSchema = {
  ...
  required: ['schemaVersion', 'package', 'from', 'to', 'output', 'summary', 'changes'],
};
...
function formatInspectSummary(result) {
  ...
  if (result.package) {
    lines.push(`📦 ${result.package}${result.version ? ` v${result.version}` : ''}`);
  }
  ...
  if (result.exports) {
    ...
    if (result.exports.classes?.length) parts.push(`${result.exports.classes.length} classes`);
    if (result.exports.objects?.length) parts.push(`${result.exports.objects.length} objects`);
  ...
  if (result.types) {
    const hasTypes = Object.values(result.types).some((v) => v && Object.keys(v).length > 0);
    if (hasTypes) {
...
const tools = [
  {
    ...
    description: 'Inspect a package to get types, exports, docs, examples, and resolution info.',
...
const server = new Server({ name: 'deplens', version: '0.1.6' }, { capabilities: { tools: {} } });
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
      ...
      const validStructured =
        typeof structuredOutput === 'object' && structuredOutput !== null
          ...
          : {
              ...
              warnings: ['Invalid output format received from runInspect'],
            };
    ...
    if (name === 'deplens.diff' || name === 'deplens_diff') {
      ...
      if (!runDiff) {
        throw new Error('Diff functionality not available. Missing diff.mjs module.');
      }
